...
def update_data_in_sqlite(db_fpath, table_name, data, id_col="created_at"):
    ...
    try:
        for row in data:
            ...
            cursor.execute(
                f'UPDATE "{table_name}" SET {set_clause} WHERE "{id_col}" = ?', values
            )
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def _push_table(db_fpath, table_name, dsn):
    ...
    try:
        ...
        for row in rows:
            ...
            upd = ", ".join(
                [f'"{c}" = EXCLUDED."{c}"' for c in cl if c != "created_at"]
            )
            cur.execute(
            ...
            lib.bs.Alert(
                ...
                lib.html.div(
                    ...
                    lib.html.div(
                        ...
                        lib.html.br(),
                        (
                            lib.html.small(style=lib.Style(color="#666"))(extra_detail)
                            if extra_detail
                            else None
                        ),
                    ),
...
def use_page_gps(lib, db_fpath, table_name, extra_fields=None):
    ...
    lon84, lat84 = (
        _merc_to_wgs84(location[0], location[1]) if location else (None, None)
    )
    gps_loc = (
        {
            "lon": f"{lon84:.6f}",
...
def make_record_manager(
    ...
    def SummaryTable():
        ...
        if not displayed_data:
            return lib.html.div(
                style=lib.Style(
                    padding="20px", textAlign="center", color="#999", fontSize="16px"
                )
        ...
        table_rows = [
            lib.html.tr(
                style=lib.Style(
                    ...
                    backgroundColor=(
                        "#e8f4f8" if record.get(id_col) in selected_rows else "#fff"
                    ),
                )
            )(
                lib.html.td(
                    style=lib.Style(
                        padding="12px", borderRight="1px solid #eee", textAlign="center"
                    )
                )(
                    lib.html.input(
                        type="checkbox",
                ...
                *[
                    lib.html.td(
                        style=lib.Style(padding="12px", borderRight="1px solid #eee")
                    )(str(record.get(field, "—")))
                    for field, _ in summary_cols
                ],
                lib.html.td(style=lib.Style(padding="12px"))(
                    str(record.get(id_col, "—"))
                ),
            )
        ...
        return lib.html.div(
            ...
            lib.html.p(
                style=lib.Style(
                    fontSize="12px", color="#666", marginBottom="15px", padding="15px"
                )
            )(
    ...
    def FormView(existing_id=None, form_edit_mode=True):
        ...
        if db["displayed_data"] and existing_id:
            selected_record_data = next(
                (
                    r
                    for r in db["displayed_data"]
                    if str(r.get(id_col)) == str(existing_id)
                ),
                None,
            )
        ...
        return lib.bs.Container(
            ...
            (
                lib.html.div(
                    ...
                    lib.bs.Button(
                        variant="secondary", onClick=lambda e: set_view_mode("list")
                    )("← Back to List"),
                )
                if existing_id
                else lib.html.div()
            ...
            lib.bs.Form(
                ...
                *form_rows,
                (
                    extra_form_content(lib, existing_id, form_edit_mode)
                ...
                (
                    lib.bs.Button(
                        ...
                        style=lib.Style(
                            ...
                            cursor=(
                                "not-allowed"
                                if db["is_loading"] or is_readonly
                                else "pointer"
                            ),
                            width="220px",
                            padding="12px 24px",
                        ...
                        (
                            ...
                            if db["is_loading"]
                            else ("💾 " if existing_id else "📤 ")
                        ),
                        (
                            "Saving..."
                            if db["is_loading"] and existing_id
                            else (
                                "Submitting..."
                                if db["is_loading"]
                                else "Save Changes" if existing_id else "Submit Form"
                            )
                        ),
                    )
                    if not is_readonly
        ...
        return lib.html.div(
            ...
            lib.tabs.Tabs(
                ...
                lib.tabs.TabPanel(
                    ...
                    if view_mode != "detail"
                    else FormView(selected_record_id, form_edit_mode=edit_mode)
                ),
            ),
        )
...
def home(lib):
    ...
    h_lon, h_lat = (
        _merc_to_wgs84(float(hx), float(hy)) if hx is not None else (None, None)
    )
    ...
    def _save_gps_to_sheets():
        ...
        try:
            ...
            data_to_sqlite(
                Path(app_workspace.path) / "map_location.sqlite",
                "Map_Location",
                [record],
            )
            set_gps_saved_msg(f"✅ Saved  {lat:+.6f}°, {lon:+.6f}°  →  Map_Location")
    ...
    def _do_push_bg():
        ...
        try:
            for tbl, res in sync_all_push(
                Path(app_workspace.path), sync_db_url
            ).items():
                log.append(
                    f"✓  {tbl}: {res['rows']} rows"
                    if res["status"] == "ok"
    ...
    def _do_pull_bg():
        ...
        try:
            for tbl, res in sync_all_pull(
                Path(app_workspace.path), sync_db_url
            ).items():
                log.append(
                    f"✓  {tbl}: {res['rows']} new rows"
                    if res["status"] == "ok"
                    else f"✗  {tbl}: {res['message']}"
                )
        ...
        lib.html.div(
            ...
            (
                lib.bs.Toast(
                    ...
                    lib.bs.ToastHeader(
                        lib.html.strong(className="me-auto")("Geolocation Error")
                    ),
                        ...
                        lib.tabs.Tabs(
                            ...
                            lib.tabs.TabPanel(
                                lib.html.div(style=lib.Style(paddingTop="10px"))(
                                    ...
                                    (
                                        lib.html.div(className="coord-card")(
                                            lib.html.div(className="coord-row")(
                                                lib.html.span(className="coord-axis")(
                                                    "X"
                                                ),
                                    ...
                                    (
                                        lib.bs.Button(
                                            ...
                                            (
                                                "Saving…"
                                                if gps_saving
                                                else "Save GPS → Map Location Sheet"
                                            ),
                                        )
                                        if location
                                        else None
                                    ...
                                    (
                                        lib.html.div(className="gps-saved-badge")(
                                            gps_saved_msg
                                        )
                                        if gps_saved_msg
                                        else None
                                ...
                                lib.html.div(style=lib.Style(paddingTop="10px"))(
                                    ...
                                    lib.html.div(
                                        style=lib.Style(
                                            ...
                                            fontWeight="600",
                                            color=(
                                                "#155724" if sync_db_url else "#856404"
                                            ),
                                            background=(
                                                "#d4edda" if sync_db_url else "#fff3cd"
                                            ),
                                    ...
                                    lib.html.div(className="sidebar-section-title")(
                                        "Tables synced"
                                    ),
                                    *[
                                        lib.html.div(
...
def chatroom(lib):
    lib.register(
        "textarea.js", "ta", host="/static/hydrogeology/js", default_export="TextArea"
    )
    app_workspace = lib.hooks.use_workspace()
    ...
    sender = lib.hooks.use_channel_layer(
        group_name="hydro_sync_chat", receiver=receive_message
    )
    ...
    def _load_last_gps():
        try:
            rows = data_from_sqlite(
                Path(app_workspace.path, "map_location.sqlite"), "Map_Location"
            )
            if rows:
                latest = rows[0]
        ...
        lib.html.div(
            ...
            lib.html.div(className="chat-root")(
                ...
                (
                    ...
                    if messages
                    else lib.html.div(className="chat-messages")(
                        lib.html.div(className="chat-empty")(
                            lib.html.div(className="chat-empty-icon")("💬"),
                            lib.html.div(className="chat-empty-text")(
...
def map_location(lib):
    ...
    form_fields = [
        ...
        [
            ("grid_east", "Grid East (°Lon)"),
            ("grid_north", "Grid North (°Lat)"),
            ("altitude", "Altitude"),
        ],
        [
        ...
        [
            ("expected_depth_to_rock_m", "Expected Depth to Rock (m)"),
            ("expected_depth_to_water_m", "Expected Depth to Water (m)"),
        ],
        [("expected_formation", "Expected Formation")],
        [
            ("expected_borehole_depth_m", "Expected Borehole Depth (m)"),
            ("accessibility_to_site", "Accessibility to Site"),
        ],
        ...
        if db["displayed_data"] and existing_id:
            selected_record_data = next(
                (
                    r
                    for r in db["displayed_data"]
                    if str(r.get("created_at")) == str(existing_id)
                ),
                None,
            )
        existing_sketch = (
            selected_record_data.get("sketch", "") if selected_record_data else ""
        )
        ...
        if result["status"] == "success":
            ...
            def _save_ai():
                try:
                        ...
                        [
                            {
                                ...
                                "gps_lat": (
                                    gps["location"]["lat"] if gps["location"] else ""
                                ),
                                "gps_lon": (
                                    gps["location"]["lon"] if gps["location"] else ""
                                ),
                            }
                        ],
                    )
                    ...
                    set_ves_ai_error(
                        f"Saved analysis but DB write failed: {str(ex)[:80]}"
                    )
        ...
        return lib.html.div(
            ...
            lib.html.div(
                ...
                lib.html.div(style=lib.Style(marginTop="20px"))(
                    ...
                    # Error box
                    (
                        lib.html.div(className="ai-error-box")(f"❌ {ves_ai_error}")
                        if ves_ai_error
                        else None
                    ),
                    ...
                    (
                        lib.html.div(className="ai-panel")(
                            lib.html.div(className="ai-panel-header")(
                                ...
                                lib.html.span(className="ai-panel-badge")(
                                    "Gemini 2.5 Flash"
                                ),
                            ),
                    ...
                    (
                        ...
                        if saved_analysis and not ves_ai_result
                        else None
                    ),
                ),
            ),
...
def resistivity_survey_form(lib):
    ...
    def update_reading(index, field, value):
        ...
        r = (
            float(new_readings[index]["reading_2"])
            if new_readings[index]["reading_2"]
            else 0
        )
    ...
    def update_mn2(value):
        ...
        for i in range(len(new_readings)):
            r = (
                float(new_readings[i]["reading_2"])
                if new_readings[i]["reading_2"]
                else 0
            )
            if r and mn2:
    ...
    for i, spacing in enumerate(log_spacings):
        if i < len(survey_data["readings"]):
            ...
            if reading["average"]:
                try:
                    plot_data.append(
                        {"depth": spacing, "resistivity": float(reading["average"])}
                    )
                except Exception:
                    pass
    ...
    def resistivity_extra(lib, existing_id, form_edit_mode):
        ...
        return lib.html.div(
            ...
            lib.html.div(
                style=lib.Style(display="flex", gap="20px", marginBottom="20px")
            )(
                lib.html.div(
                    style=lib.Style(
                        flex="0 0 550px",
                            ...
                            *[
                                lib.html.tr(
                                    ...
                                    lib.html.td(
                                        ...
                                        lib.html.input(
                                            type="number",
                                            value=survey_data["readings"][idx][
                                                "reading_2"
                                            ],
                                            style=lib.Style(width="90%", padding="4px"),
                                            placeholder="0.0",
                                            disabled=is_readonly,
                                            onChange=lambda e, i=idx: update_reading(
                                                i, "reading_2", e.target.value
                                            ),
                                        ),
                                    ),
                                    lib.html.td(
                    ...
                    lib.html.div(style=lib.Style(height="700px"))(
                        lib.pl.Plot(
                            ...
                            layout=lib.Props(
                                ...
                                xaxis=lib.Props(
                                    title=lib.Props(
                                        text="Apparent Resistivity ρa (Ω·m)"
                                    ),
                                    type="log",
                                    autorange="reversed",
                                ),
            ...
            lib.html.div(
                ...
                # Error
                (
                    lib.html.div(className="ai-error-box")(f"❌ {res_ai_error}")
                    if res_ai_error
                    else None
                ),
                ...
                (
                    ...
                    if saved_analysis and not res_ai_result
                    else None
                ),
            ),
        )
...
def image_analysis(lib):
    ...
    async def handle_file_upload(e):
        ...
        result = await analyze_rock_from_bytes(
            gemini_api_key, data_bytes, mime_type=mime_type
        )
        ...
        if len(selected_rows) == 1:
            ...
            record = next(
                (
                    r
                    for r in db["displayed_data"]
                    if str(r.get("created_at")) == str(rid)
                ),
                None,
            )
            if record:
    ...
    def ArchiveList():
        ...
        if not data:
            return lib.html.div(
                style=lib.Style(
                    padding="40px", textAlign="center", color="#999", fontSize="16px"
                )
        ...
        rows = [
            lib.html.tr(
                style=lib.Style(
                    ...
                    backgroundColor=(
                        "#e8f4f8"
                        if record.get("created_at") in selected_rows
                        else "#fff"
                    ),
                )
            )(
                lib.html.td(style=lib.Style(padding="10px", textAlign="center"))(
                    lib.html.input(
                        ...
                        onChange=lambda e, rid=record.get("created_at"): toggle_row(
                            rid
                        ),
                        style=lib.Style(cursor="pointer", width="16px", height="16px"),
                    )
                ),
                lib.html.td(style=lib.Style(padding="10px"))(
                    str(record.get("village", "—"))
                ),
                lib.html.td(style=lib.Style(padding="10px"))(
                    str(record.get("formation", "—"))
                ),
                lib.html.td(
                    style=lib.Style(
                        padding="10px",
                ...
                lib.html.td(
                    style=lib.Style(padding="10px", fontSize="12px", color="#666")
        ...
        return lib.html.div(
            ...
            lib.html.div(
                ...
                lib.html.table(
                    style=lib.Style(width="100%", borderCollapse="collapse")
                )(
                    lib.html.thead(
                        style=lib.Style(
                            backgroundColor="#f5f5f5", borderBottom="2px solid #ddd"
                        ...
                        lib.html.tr(
                            ...
                            lib.html.th(
                                style=lib.Style(padding="10px", textAlign="left")
                            )("Saved At"),
                        )
                    ),
                    lib.html.tbody(*rows),
    ...
    def ArchiveDetail():
        ...
        return lib.html.div(style=lib.Style(padding="20px", maxWidth="860px"))(
            lib.html.div(
                ...
                lib.bs.Button(
                    variant="danger", onClick=lambda e: set_delete_confirm(True)
            ...
            lib.html.div(
                ...
                lib.html.div(style=lib.Style(flex="0 0 380px", minWidth="260px"))(
                    lib.html.h5(style=lib.Style(marginBottom="10px", color="#333"))(
                        "📷 Rock Image"
                    ),
                    (
                        lib.html.img(
                ...
                lib.tabs.TabPanel(
                    lib.html.div(
                        ...
                        (
                            ...
                            else lib.bs.Form(onSubmit=handle_save_analysis)(
                                lib.bs.Button(
